...
export type CommandHandler = (args: string[], fs: FileSystem) => CommandResult
...
export const commands: Record<string, CommandHandler> = {
  ...
  cd: (args, fs) => {
    ...
    if (!success) return err(`cd: ${target}: No such file or directory`)
    return ok('')
  },
  ...
  cat: (args, fs) => {
    ...
    if (!file) return err(`cat: ${args[0]}: No such file`)
